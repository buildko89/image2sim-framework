...
## cat_koha9.glb のインタラクティブビューア。
## 実行: godot --path . res://ViewKoha9.tscn
## キー: 1-8 = アニメーション切替, Space = 次, R = 回転ON/OFF, Esc = 終了
...
func _ready() -> void:
	var packed_scene := load("res://cat_koha.glb")
	...
	guide.text = "1-8: アニメ切替 / Space: 次 / R: 回転 / Esc: 終了\n1:Idle1 2:Idle2 3:Walk 4:Walkback 5:Run 6:Jump 7:Atk1 8:Atk2"
	ui.add_child(guide)
...
func _play(index: int) -> void:
	...
	if _player.has_animation(anim_name):
		# Jump はルートモーション入りなので、位置がリセットされるようにシークしてから再生
