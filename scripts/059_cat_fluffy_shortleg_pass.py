...
DEFAULT_INPUT_BLEND = "output/rigged/cat_visual_pass.blend"
DEFAULT_OUTPUT_BLEND = "output/rigged/cat_fluffy_shortleg_pass.blend"
DEFAULT_OUTPUT_GLB = "output/clean_3d/cat_fluffy_shortleg_pass.glb"
DEFAULT_OUTPUT_FBX = "output/clean_3d/cat_fluffy_shortleg_pass.fbx"
DEFAULT_REPORT = "output/reports/task59_cat_fluffy_shortleg_pass.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run short-leg and fluffy silhouette cat pass in Blender.")
...
def main() -> int:
    ...
    blender_script = repo_root / "blender" / "cat_fluffy_shortleg_pass.py"
    ...
    plan = {
        ...
        "steps": [
            "open the photo-based visual pass blend",
            "compress leg vertices strongly toward the feet",
            "lower and widen body/shoulders to hide more leg length under long fur",
            "add a duplicated skinned fur shell while preserving calico material assignments",
            "export animated GLB/FBX for Godot review",
        ],
    }
    write_json(report.with_name("task59_cat_fluffy_shortleg_pass_plan.json"), plan)

    print("Task 5.9 fluffy shortleg pass")
