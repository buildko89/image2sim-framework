...
DEFAULT_INPUT_BLEND = "output/rigged/cat_shape_pass.blend"
DEFAULT_OUTPUT_BLEND = "output/rigged/cat_visual_pass.blend"
DEFAULT_OUTPUT_GLB = "output/clean_3d/cat_visual_pass.glb"
DEFAULT_OUTPUT_FBX = "output/clean_3d/cat_visual_pass.fbx"
DEFAULT_REPORT = "output/reports/task58_cat_visual_pass.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run photo-based cat visual improvement pass in Blender.")
...
def main() -> int:
    ...
    blender_script = repo_root / "blender" / "cat_visual_pass.py"
    ...
    plan = {
        ...
        "steps": [
            "open the current cat shape pass blend",
            "make the body wider/lower to read as a long-haired cat",
            "round the head and shorten the muzzle impression further",
            "lower/narrow fox-like ears",
            "make the tail longer and fuller to match the reference cat plume tail",
            "reassign materials to a mostly-white calico pattern based on photo references",
            "export animated GLB/FBX for Godot review",
        ],
    }
    write_json(report.with_name("task58_cat_visual_pass_plan.json"), plan)

    print("Task 5.8 cat visual pass")
