"""P4-顔 (koha9face): 実物の顔写真（input/glb確認/顔検討.png 右）に目を寄せる。

p3_paint_texture_koha9.py の face 変種。柄は koha9 と同一で、**目だけ**を変える:
  - 目の拡大（配色.pdf p8 対応の x1.38）を撤回し、むしろ縮小（横 0.95 / 縦 0.72）。
    実物は細いアーモンド形で、見開いた丸い目ではない。
  - 保護楕円を小さく・中心を 1.5mm 下げる → 目の上部が顔の毛色で塗られ、
    上まぶたが被さった「伏し目」になる。
  - 虹彩を明るい黄緑 → 淡い青磁グリーンへ。
p3_koha9_* は正典として残し、このスクリプトは p3_koha9face_* だけを出力する。
...
OUT_RGB = Path("output_v2/textures/p3_koha9face_basecolor.png")
OUT_RGBA = Path("output_v2/textures/p3_koha9face_basecolor_alpha.png")
DEBUG = Path("output_v2/reports/p3_koha9face_texture_debug.png")
...
# face: 保護楕円を小さく・中心を 1.5mm 下げる。上へはみ出た目の絵が顔の毛色で
# 塗られ、上まぶたが被さった細い目になる（実物は伏し目がちのアーモンド形）。
# 素材の目は左右非対称（罠⑱）。+X の目は絵が大きく、同じ半径だと丸目が残る（実測）
# ので +X だけさらに絞る。
# P4-顔第2段階のメッシュ変形で表面が動いたため、新旧 npz の同一テクセル対応で再マップ
# （目は外へ x1.076・前へ +0.9mm。X 半径も同率で拡大）。2026-07-15
# 第2ラウンド: 縦半径を広げて目を大きく（0.0058/0.0050 → 0.0072/0.0062）
EYES = [((-0.0212, 0.2092, 0.1902), (0.0103, 0.0072, 0.0092)),
        ((+0.0217, 0.2098, 0.1886), (0.0099, 0.0062, 0.0090))]

# face 第2ラウンド（顔検討2.png）: 「もう少し大きく、釣り目ぎみ」→ 絵を少し拡大し、
# 保護楕円の開きを縦に広げ、楕円全体を EYE_TILT で傾ける（外眼角側が上がる）。
EYE_ART_MAJOR = 1.06
EYE_ART_MINOR = 0.84
EYE_ART_SPAN = 2.4     # 元の大きい絵の範囲まで塗り直すので広めに取る
EYE_LINER_R = (0.86, 1.10)
EYE_TILT = 0.18        # 釣り目の傾き（ラジアン ≈ 10°）。符号は目ごとに外側が上がる向き
# face: 実物の虹彩は淡い青磁グリーン（顔検討.png）。乗算ティントでは素材の
# 黄色い部分が黄色のまま残る（実測）ので、目標色への部分置換で寄せる。
IRIS_CELADON = np.array([175, 190, 152], float)
IRIS_MIX = 0.55
IRIS_TINT = np.array([1.0, 1.0, 1.0], float)   # 置換方式に切り替えたので無効化

# face 第2段階: 実物（顔検討.png 右）の鼻は大きめのピンクの三角形。素材の鼻の絵を
# テクセル空間で等方拡大する。中心はピンクのテクセルの重心（目の罠⑱と同じ理由）。
# 中心座標は再マップ済みの新メッシュ表面（鼻先 Z=0.2227）。
NOSE = ((0.0, 0.196, 0.2227), (0.011, 0.010, 0.008))
NOSE_ART_SCALE = 1.3
...
FACE_BLOBS = [
    ...
    # 顔第2段階のメッシュ変形（頬拡幅・耳を外へ低く・マズル+8.3mm）に合わせて
    # 各アンカーを新旧 npz の同一テクセル対応で再マップ済み。2026-07-15
    ("頭頂の茶",             (-0.0113, 0.2450, 0.1500), (0.040, 0.030, 0.040), CROWN_RUST, None),
    ("+X 頭の黒帯(耳へ)",    (+0.0323, 0.2400, 0.1602), (0.022, 0.028, 0.038), FACE_DARK, None),
    # --- 額の前(耳の間)を横切るこげ茶の帯。白いブレーズはこの下で終わる ---
    ("額の前のこげ茶の帯",   (+0.0043, 0.2450, 0.1764), (0.030, 0.016, 0.020), FACE_DARK, None),
    # --- +X(猫の左): 耳の下から眉・目尻へ続く広いこげ茶の帯 ---
    ("+X 顔の帯 眉〜目尻",   (+0.0373, 0.2240, 0.1763), (0.015, 0.024, 0.028), FACE_DARK, None),
    # 耳の付け根 +X の実測: X 0.022〜0.042, Y 0.230〜0.260, Z 0.130〜0.166
    ("+X 耳の下の黒",        (+0.0392, 0.2420, 0.1480), (0.014, 0.016, 0.020), FACE_BLACK, None),
    # --- -X(猫の右): 眉の赤茶の斑 + 顔の外側のこげ茶 ---
    ("-X 眉の赤茶",          (-0.0269, 0.2240, 0.1869), (0.013, 0.014, 0.016), FACE_RUST, None),
    ("-X 顔の外側のこげ茶",  (-0.0480, 0.2200, 0.1660), (0.013, 0.024, 0.028), FACE_DARK, None),
    # --- 耳の後ろ ---
    ("-X 耳の後ろ: 茶",      (-0.0419, 0.2639, 0.1440), (0.018, 0.017, 0.015), EAR_BROWN, "back"),
    ("+X 耳の後ろ: こげ茶",  (+0.0412, 0.2642, 0.1440), (0.018, 0.017, 0.015), EAR_DARK, "back"),
    # --- 耳の前（内側）: 肌色。柄の後に置いて必ず上に来るようにする ---
    ("-X 耳の前: 肌色",      (-0.0419, 0.2639, 0.1440), (0.018, 0.017, 0.015), EAR_FLESH, "front"),
    ("+X 耳の前: 肌色",      (+0.0412, 0.2642, 0.1440), (0.018, 0.017, 0.015), EAR_FLESH, "front"),
    # --- そばかす: 鼻のすぐ脇（+X 側）。p2 のリファレンスにもある ---
    ("+X マズルのそばかす",  (+0.0179, 0.1948, 0.2205), (0.0066, 0.0050, 0.0060), FRECKLE, None),
]
...
def eye_norm_dist(pos, center, radii):
    """目の楕円距離。X-Y 面内で EYE_TILT だけ傾ける（外眼角側が上がる = 釣り目）。

    +X の目は +θ（+X 端が上がる）、-X の目は -θ（-X 端が上がる）。"""
    d = pos - np.array(center)
    th = EYE_TILT * (1.0 if center[0] >= 0.0 else -1.0)
    ct, st = np.cos(th), np.sin(th)
    u = d[..., 0] * ct + d[..., 1] * st
    w = -d[..., 0] * st + d[..., 1] * ct
    dr = np.stack([u, w, d[..., 2]], axis=-1) / np.array(radii)
    return np.sqrt((dr ** 2).sum(-1))


def eye_blob_alpha(pos, center, radii):
    return smoothstep(1.02, 0.72, eye_norm_dist(pos, center, radii))


def eye_ring_alpha(pos, center, radii, r0, r1):
    f = eye_norm_dist(pos, center, radii)
    return smoothstep(r0, r0 + 0.10, f) * smoothstep(r1, r1 - 0.15, f)
...
def magnify_eye_art(rgb: np.ndarray, pos: np.ndarray) -> np.ndarray:
    ...
    out = rgb.copy()
    for c, rad in EYES:
        span = eye_blob_alpha(pos, c, tuple(r * EYE_ART_SPAN for r in rad)) > 0.01
        if not span.any():
            continue
...
def magnify_nose_art(rgb: np.ndarray, pos: np.ndarray) -> np.ndarray:
    """素材の鼻の絵（ピンク）をテクセル空間で等方拡大する。中心はピンクの重心。"""
    N = rgb.shape[0]
    span = blob_alpha(pos, NOSE[0], NOSE[1]) > 0.01
    if not span.any():
        print("  [warn] 鼻の拡大: span が空。NOSE 座標を確認すること")
        return rgb
    r, g, b = rgb[..., 0], rgb[..., 1], rgb[..., 2]
    pink = span & (r > 140) & ((r - g) > 25) & ((g - b) < 20)
    if pink.sum() < 20:
        print(f"  [warn] 鼻の拡大: ピンクのテクセルが {int(pink.sum())} 個しかない")
        return rgb
    py, px = np.nonzero(pink)
    cy, cx = py.mean(), px.mean()
    ii, jj = np.meshgrid(np.arange(N), np.arange(N), indexing="ij")
    si = np.clip(np.rint(cy + (ii - cy) / NOSE_ART_SCALE).astype(int), 0, N - 1)
    sj = np.clip(np.rint(cx + (jj - cx) / NOSE_ART_SCALE).astype(int), 0, N - 1)
    out = np.where(span[..., None], rgb[si, sj], rgb)
    print(f"  鼻の絵を拡大: x{NOSE_ART_SCALE} 中心=({cy:.0f},{cx:.0f}) "
          f"ピンク {int(pink.sum()):,} テクセル")
    return out
...
def main() -> int:
    ...
    for c, rad in EYES:
        keep_eye = np.maximum(keep_eye, eye_blob_alpha(pos, c, rad))
    ...
    for c, rad in EYES:
        liner = np.maximum(liner, eye_ring_alpha(pos, c, rad, *EYE_LINER_R))
    a = liner[..., None]
    ...
    # face: 虹彩（黄〜緑の明るいテクセル。瞳孔・アイラインの暗部と白いハイライトは除外）を
    # 淡い青磁グリーンへ部分置換する。陰影は 45% 残る。
    r_, g_, b_ = rgb[..., 0], rgb[..., 1], rgb[..., 2]
    iris = (keep_eye > 0.5) & (g_ > 110) & (g_ > b_ + 20) & (r_ > 80)
    src_keep = np.where(iris[..., None],
                        rgb * (1 - IRIS_MIX) + IRIS_CELADON * IRIS_MIX, rgb)
    print(f"  目 虹彩を青磁グリーンへ置換: {int(iris.sum()):,} テクセル")
